"use client";
import { skills } from "@/app/assets";
import Tilt from "react-parallax-tilt";

const Skills = () => {
  return (
    <div id="skills" className="text-white py-16 scroll-mt-35">
      <h1
        className="text-center text-2xl md:text-4xl xl:text-5xl font-bold text-white"
        data-aos="fade-up"
        data-aos-delay="100"
      >
        My <span className="text-cyan-200">Skills</span>
      </h1>
      <div className="flex flex-wrap justify-center gap-6 mt-16">
        {skills.map((skill) => (
          <Tilt key={skill.id} scale={1.5} transitionSpeed={400}>
            <div
              className="bg-col-4 text-center w-40 h-48 rounded-3xl flex flex-col items-center justify-center shadow-lg transition-all duration-300 hover:scale-105"
              data-aos="flip-right"
              data-aos-delay={skill.id * 100}
            >
              <div className="text-5xl mb-4 text-gray-300">{skill.icon}</div>
              <p className="text-2xl font-semibold">{skill.percentage}</p>
              <p className="text-purple-400 mt-1">{skill.name}</p>
            </div>
          </Tilt>
        ))}
      </div>
    </div>
  );
};

export default Skills;
